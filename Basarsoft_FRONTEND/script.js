document.addEventListener('DOMContentLoaded', (event) => {
    ...
    function loadWktData() {
        fetch('https://localhost:7037/point') // Sunucuya WKT verilerini almak için istek gönder
            .then(response => {
                if (!response.ok) {
                    ...
                    if (feature.getGeometry().getType() === 'Point') {
                        style = new ol.style.Style({
                            ...
                            text: new ol.style.Text({
                                ...
                                overflow: true,
                                fill: new ol.style.Fill({
                                    color: '#0044cc' // Mavi etiket rengi
                                }),
                                stroke: new ol.style.Stroke({
                                    color: '#000', // Siyah kenar rengi
                        ...
                        style = new ol.style.Style({
                            ...
                            text: new ol.style.Text({
                                ...
                                overflow: true,
                                fill: new ol.style.Fill({
                                    color: '#000' // Siyah etiket rengi
                                }),
                                stroke: new ol.style.Stroke({
                                    color: '#0044cc', // Mavi kenar rengi
                                    width: 3 // Kenar kalınlığı
                                }),
                                text: item.name
                            })
                        ...
                        style = new ol.style.Style({
                            ...
                            fill: new ol.style.Fill({
                                color: 'rgba(0, 68, 204, 0.3)' // Yarı şeffaf mavi renk
                            }),
                            text: new ol.style.Text({
                                font: '12px Calibri,sans-serif',
                                overflow: true,
    ...
    function addDrawInteraction(type) {
        ...
        drawInteraction.on('drawend', function(evt) {
            ...
            fetch('https://localhost:7037/point', {
                method: 'POST',
                headers: {
            ...
            if (data.status) {
                ...
                pointsListPanel = jsPanel.create({
                    ...
                    callback: function() {
                        ...
                        this.header.style.backgroundColor = '#0044cc'; 
...
function deletePoint(id) {
    fetch(`https://localhost:7037/point/${id}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(() => {
        // Silinen öğeyi vektör kaynağından kaldır
        const feature = vectorSource.getFeatureById(id);
        if (feature) {
            vectorSource.removeFeature(feature);
            vectorSource.changed(); // Değişikliği bildir
        }

        pointsListPanel.close();
        showPointsInPanel(); // Noktaları yeniden listele
    })
    .catch(error => {
        console.error('Error:', error);
...
async function updatePoint(id, newName) {
    try {
        ...
        const response = await fetch(`https://localhost:7037/point/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        // Send updated data
        const updateResponse = await fetch(`https://localhost:7037/point/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
